...
const loadFonts = async () => {
    try {
        ...
        console.log('Tipografias obtenidas correctamente', data.data[0]);
    } catch (error) {
...
<template>
    <article class="w-full h-145 my-2 p-3 overflow-y-auto flex flex-col items-center gap-3 max-h-140">
        <div v-for="font in tipographyData" :key="font.id_tipography" class="flex justify-around items-center w-full h-1/8 border-b-2 border-blue-800" :style="font.is_selected === 1 ? 'background-color: #BFCEDF;' : ''">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                    ...
                    class="lucide lucide-trash2-icon lucide-trash-2 hover:text-red-600 transition-colors duration-300 cursor-pointer" :style="font.is_selected === 1 ? 'visibility: hidden;' : ''">
                    <path d="M3 6h18" />
