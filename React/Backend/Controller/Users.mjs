import {
  ...
  validateUpdateUser,
  validateRegister,
} from "../Validations/SchemaUser.mjs";
...
export class UsersControllers {
  ...
  getUserByUsername = async (req, res) => {
    try {
      ...
      const userUsername = await this.ModelsUsers.getUserByUsername({
        username,
      });
      if (userUsername) {
        return res.status(200).json({
  ...
  updateUser = async (req, res) => {
    try {
      ...
      const user = await this.ModelsUsers.updateUser({
        id_user,
        user: result.data,
      });
      return res.status(200).json({
  ...
  activateUser = async (req, res) => {
    try {
      const { id_user } = req.params;
      const user = await this.ModelsUsers.activateOrDeactiveUser({
        id_user,
        isActive: 1,
      });
      if (user) {
